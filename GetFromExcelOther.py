...
def read_excel():
    ...
    for i in range(1, 6):
        col_arr.append(sheet.cell(row=1, column=i).value)
    for row_val in range(2, max_row + 1):
        for col_val in range(1, 6):
            excel_obj[col_arr[col_val - 1] + str(row_val)] = sheet.cell(row=row_val, column=col_val).value
    print(excel_obj)
